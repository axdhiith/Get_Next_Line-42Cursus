...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/20 14:01:01 by alakshmi          #+#    #+#             */
/*   Updated: 2023/03/10 21:02:56 by alakshmi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE < 0 || fd < 0 || fd >= 1024)
		return (NULL);
	bufferstr[fd] = readline(fd, bufferstr[fd]);
	if (!bufferstr[fd])
		return (NULL);
	line = remline(bufferstr[fd]);
	bufferstr[fd] = trimbuffer(bufferstr[fd]);
	return (line);
}
